...
sequence_statistics2.0.pl - sequence information statistics

=head1 SYNOPSIS

USAGE: 

  sequence_statistics2.0.pl <input_file[,input_file1,...]>
...
sub output{
	my $filename = shift;
	if ($options{type} =~ /fasta/){
		printf STDOUT "%15s\t%15s\t%15s\t%15s\t%15s\t%15s\t%15s\t%15s\n", 'File name', 'Seq number', 'Total length', 'GC content(%)', 'Gap length', 'Gap rate(%)', 'N50 length', 'N90 length' if (defined $options{head});
		printf STDOUT "%15s\t%15d\t%15d\t%15.2f\t%15d\t%15.2f\t%15d\t%15d\n", $filename, $seqnum, $sumlength, $GCp, $sumN, $Np, $N50, $N90;
	}
	if ($options{type} =~ /fastq/){
		if (defined $options{reflength}){
			printf STDOUT "%15s\t%15s\t%15s\t%15s\t%15s\t%15s\t%15s\n", 'File name', 'Read number', 'Clean Base', 'GC content(%)', 'Gap length', 'Gap rate(%)', 'Coverage' if (defined $options{head});
			printf STDOUT "%15s\t%15d\t%15d\t%15.2f\t%15d\t%15.2f\t%15.2f\n", $filename, $seqnum, $sumlength, $GCp, $sumN, $Np, $coverage;
		}else{
			printf STDOUT "%15s\t%15s\t%15s\t%15s\t%15s\t%15s\n", 'File name', 'Read number', 'Clean Base', 'GC content(%)', 'Gap length', 'Gap rate(%)' if (defined $options{head});
			printf STDOUT "%15s\t%15d\t%15d\t%15.2f\t%15d\t%15.2f\n", $filename, $seqnum, $sumlength, $GCp, $sumN, $Np;
		}
	}
}
